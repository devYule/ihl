...
import com.yule.open.utils.JDatabaseConverter;
    ...
    private FieldSpec.Builder generateSpec(Column column, String refEntity) {
        ...
        if (column.isFK()) {
            return FieldSpec.builder(
                    JDatabaseConverter.convert(refEntity),
                    fieldName,
                    Modifier.PRIVATE
            );
        }
        ...
        return FieldSpec.builder(
                JDatabaseConverter.convert(column.getDataType(), column.getDataLenNum()),
                fieldName,
                Modifier.PRIVATE
        );
    }
            ...
            if (ts[f.getParent()].getPkCnt() > 1) {
                ...
                if (!defaultType.isAddedEmbeddableAnnotation()) {
                    ...
                    FieldSpec embeddedId =
                            FieldSpec.builder(JDatabaseConverter.convert(IHLCore.nameGenerator.generateEntityName(defaultType.getAdditionalTypeName(embeddable))),
